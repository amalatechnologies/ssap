    ...
    <v-card color="transparent" flat>
      <v-toolbar color="primary" tile dark flat>
        ...
        <v-toolbar-title class="white--text">
          &nbsp; &nbsp; {{ capitalizeFirstLetter(request.clientName) }} Guarantor Details</v-toolbar-title>
      </v-toolbar>

      <v-card class="mt-5 pa-4">
        <v-row>
          <v-col cols="8">Client Name:</v-col>
          <v-col cols="4" class="d-flex justify-start">{{ request.clientName }}</v-col>
          <v-col cols="8">Loan product:</v-col>
          <v-col cols="4" class="d-flex justify-start">{{ request.productName }}</v-col>
          <v-col cols="8">Loan Account:</v-col>
          <v-col cols="4" class="d-flex justify-start">{{ request.loanAccount }}</v-col>
          <v-col cols="8">Loan Amount:</v-col>
          <v-col cols="4" class="d-flex justify-start">{{ request.loanAmount }}</v-col>
          <v-col cols="8">Requested Amount:</v-col>
          <v-col cols="4" class="d-flex justify-start">{{ request.totalGuarantee }}</v-col>
        </v-row>
      </v-card>
      ...
      <v-card v-for="(d, index) in details" :key="index" class="mt-5 pa-4" color="#FAFAFA" tile>
        <v-card-text>
          <v-row class="black--text" dense>
            <v-col cols="8">Client Name:</v-col>
            <v-col cols="4" class="d-flex justify-start">{{ d.displayName }}</v-col>
            <v-col cols="8">Product Name:</v-col>
            <v-col cols="4" class="d-flex justify-start">{{ d.loanProductName }}</v-col>
            <v-col cols="8">Account:</v-col>
            <v-col cols="4" class="d-flex justify-start">{{ d.accountNumber }}</v-col>
            <v-col cols="8">Loan Amount:</v-col>
            <v-col cols="4" class="d-flex justify-start">{{ d.loanAmount }}</v-col>
            <v-col cols="8">Guarantee:</v-col>
            <v-col cols="4" class="d-flex justify-start">{{ d.guaranteeAmount }}</v-col>
          </v-row>
        </v-card-text>
...
export default {
  ...
  methods: {
    ...
    async retrieveguarantordetails() {
      await this.$api.$get(`loans/client/${this.request.guarantorId}/loan/${this.request.loanId}/obligeedetails`)
        .then(response => {
